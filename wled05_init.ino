...
void wledInit()
{
    ...
    if (onlyAP || strlen(apSSID) > 0) {
        ...
        dnsServer.start(53, "luminxa.me", WiFi.softAPIP());
        dnsActive = true;
    }

    prepareIds(); //UUID from MAC (for Alexa and MQTT)
    if (strcmp(cmDNS, "x") == 0) { //fill in unique mdns default
        strcpy(cmDNS, "luminxa-");
        strcat(cmDNS, escapedMac.c_str());
    }
    if (mqttDeviceTopic[0] == 0) {
        strcpy(mqttDeviceTopic, "luminxa/");
    ...
    if (!onlyAP) {
        ...
        if (strlen(cmDNS) > 0 && !onlyAP) {
            ...
            MDNS.addService("luminxa", "tcp", 80);
        }
        strip.service();
...
{
    bool set = apSSID[0];
    if (!set) strcpy(apSSID, "LuminxaNode1");
    WiFi.softAP(apSSID, apPass, apChannel, apHide);
    // WiFi.softAP("LuminxaNode1", NULL, 0);
    Serial.println("Begin Ap mode...");
    if (!set) apSSID[0] = 0;
}
